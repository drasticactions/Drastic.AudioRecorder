...
public partial class AudioRecorderService
{
    ...
    /// </summary>
    public static void RequestAVAudioSessionCategory(
        AVAudioSessionCategory category = AVAudioSessionCategory.PlayAndRecord)
    {
    ...
    void OnRecordingStarting()
    {
        if (requestedAVAudioSessionCategory.HasValue)
        {
            ...
            if (!audioSession.Category.ToString().EndsWith(requestedAVAudioSessionCategory.Value.ToString()))
            {
                ...
                {
                    throw new Exception(
                        $"Current AVAudioSession category is ({currentAVAudioSessionCategory}); Application requested an AVAudioSession category of {requestedAVAudioSessionCategory.Value} but received error when attempting to set it: {err}");
                }
            }
    ...
    void OnRecordingStopped()
    {
        if (currentAVAudioSessionCategory != null)
        {
            ...
            else
            {
                ...
                Debug.WriteLine(
                    $"Error attempting to set the AVAudioSession category back to {currentAVAudioSessionCategory} :: {err}");
            }
