...
internal class AudioStream : IAudioStream
{
    ...
    void BufferOperation(Func<AudioQueueStatus> bufferFn, Action successAction = null,
        Action<AudioQueueStatus> failAction = null)
    {
        var status = bufferFn();
        ...
        else
        {
            ...
            {
                throw new Exception(
                    $"AudioStream buffer error :: buffer operation returned non - Ok status:: {status}");
            }
        }
    }
        ...
        if (audioQueue != null)
        {
            ...
            if (audioQueue.IsRunning)
            {
                BufferOperation(() => audioQueue.Stop(true),
                    ...
                    status => Debug.WriteLine("AudioStream.Stop() :: audioQueue.Stop returned non OK result: {0}",
                        status));
            }
    ...
    void InitAudioQueue()
    {
        ...
        var audioFormat =
            AudioStreamBasicDescription.CreateLinearPCM(SampleRate, (uint)ChannelCount, (uint)BitsPerSample);
        ...
        var bufferByteSize =
            (int)(TargetMeasurementTime / 1000F /*ms to sec*/ * SampleRate * audioFormat.BytesPerPacket);
        ...
        for (var index = 0; index < CountAudioBuffers; index++)
        {
            ...
            BufferOperation(() => audioQueue.AllocateBuffer(bufferByteSize, out bufferPtr),
                () =>
                {
                    BufferOperation(() => audioQueue.EnqueueBuffer(bufferPtr, bufferByteSize, null),
                        () => Debug.WriteLine("AudioQueue buffer enqueued :: {0} of {1}", index + 1,
                            CountAudioBuffers));
                });
        }
    }
        ...
        try
        {
            ...
            if (e.Buffer.AudioDataByteSize > 0)
            {
                ...
                if (Active)
                {
                    ...
                    {
                        Debug.WriteLine(
                            "AudioStream.QueueInputCompleted() :: audioQueue.EnqueueBuffer returned non-Ok status :: {0}",
                            status);
                        OnException?.Invoke(this,
                            new Exception($"audioQueue.EnqueueBuffer returned non-Ok status :: {status}"));
                    });
                }
            }
        }
