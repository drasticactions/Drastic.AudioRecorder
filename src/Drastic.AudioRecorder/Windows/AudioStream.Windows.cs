    ...
    async Task Init()
    {
        try
        {
            ...
            if (graphResult.Status == AudioGraphCreationStatus.Success)
            {
                ...
                var inputResult =
                    await audioGraph.CreateDeviceInputNodeAsync(MediaCategory.Communications, pcmEncoding);
                ...
                {
                    throw new Exception(
                        $"audioGraph.CreateDeviceInputNodeAsync() returned non-Success status: {inputResult.Status}");
                }
            }
            else
            {
    ...
    unsafe void Graph_QuantumStarted(AudioGraph sender, object args)
    {
        ...
        try
        {
            ...
            using (IMemoryBufferReference reference = buffer.CreateReference())
            {
                ...
                {
                    audioBytes =
                        new byte [buffer.Length * broadcastSize /
                                  2]; // buffer length * # of frames we want to accrue / 2 (because we're transforming float audio to Int 16)
                }
