// <copyright file="AudioFunctions.cs" company="Drastic Actions">
...
{
    private static readonly float MAX8BITSSIGNED = byte.MaxValue;
    private static readonly float MAX8BITSUNSIGNED = 0xff;
    private static readonly float MAX16BITSSIGNED = short.MaxValue;
    private static readonly float MAX16BITSUNSIGNED = 0xffff;
    ...
    internal static void WriteWavHeader(BinaryWriter writer, int channelCount, int sampleRate, int bitsPerSample, int audioLength = -1)
    {
        ...
        if (audioLength > -1)
        {
            writer.Write((int)(audioLength - 8)); // Size of the overall file - 8 bytes, in bytes (32-bit integer).
        }
        else
        {
...
#pragma warning restore CS0675 // Bitwise-or operator used on a sign-extended operand
        ...
        if (signed)
        {
            if (use16Bit)
            {
                level = (float)max / MAX16BITSSIGNED;
            }
            else
            {
                level = (float)max / MAX8BITSSIGNED;
            }
        }
        else
        {
            if (use16Bit)
            {
                level = (float)max / MAX16BITSUNSIGNED;
            }
            else
            {
                level = (float)max / MAX8BITSUNSIGNED;
            }
        }
